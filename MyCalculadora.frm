...
Private Sub Command1_Click()
    ...
    If txt1 = "" And txt2 = "" Then
       GoTo pular
    ElseIf (Not IsNumeric(txt1) And txt1 <> "") Or (Not IsNumeric(txt2) And txt2 <> "") Then
       MsgBox "Atenção!, Não se pode calcular um texto. Favor verificar.", vbInformation
       GoTo pular
    ElseIf optAdicao Then
       vValor = Val(txt1) + Val(txt2)
    ElseIf optSubtracao Then
       vValor = Val(txt1) - Val(txt2)
    ElseIf optMultiplicacao Then
       vValor = Val(txt1) * Val(txt2)
    ElseIf optExponenciacao Then
       vValor = Val(txt1) ^ Val(txt2)
    ElseIf optRestoDivisao Then
